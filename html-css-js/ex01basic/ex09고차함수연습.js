...
dData.forEach((data) => {
  let cnt = 0;
  let sum = eData.reduce((p, n) => {
    if (data.no === n.dno) {
      let salary = parseInt(n.salary);
      cnt += 1;
      return p + salary;
    }
    return p;
  ...
  if (sum ?? 0) {
    console.log(`${data.no} 평균 : ${Math.floor(sum / cnt)}`);
  }
});
...
dData.forEach((data) => {
  ...
  console.log(`${data.no} 받는인원수 : ${cnt}`);
});
console.log("--------------3--------------");
// 3.부서별 최대 급여와 최소 급여
dData.forEach((data) => {
  let list = eData.filter((no) => no.dno === data.no);
  let dataList = [];
  for (let i = 0; i < list.length; i += 1) {
    dataList[i] = parseInt(list[i].salary);
  }
  if (dataList.length !== 0) {
    console.log(
      `${data.no} Max : ${Math.max(...dataList)} Min : ${Math.min(...dataList)}`
    );
  }
});
console.log("--------------4--------------");
// 4.부서번호가 30번인 사원들의 이름, 직급, 부서번호, 부서위치를 조회하시오.
let location = dData.find((data) => (data.no === "30" ? data.location : null));
eData.forEach((data) => {
  if (data.dno === "30") {
    console.log(
      `이름 : ${data.ename} 직급 : ${data.title} 부서번호 : ${data.dno} 부서위치 : ${location.location}`
    );
  }
});
console.log("--------------5--------------");
...
dData.forEach((data) => {
  let edata = eData.forEach((d) => {
    if (data.no === d.dno && d.commission !== "0") {
      console.log(
        `이름 : ${d.ename} 커미션 : ${d.commission} 부서이름 : ${data.dname} 부서위치 : ${data.no}`
      );
    }
  });
});
...
let edata = eData.sort((a, b) => {
  if (parseInt(a.salary) < parseInt(b.salary)) {
    return 1;
  } else if (parseInt(a.salary) > parseInt(b.salary)) {
    return -1;
  } else {
    if (a.ename < b.ename) {
      return -1;
...
let ddata = dData.find((data) => data.location === "DALLAS");
eData.forEach((data) => {
  if (data.dno === ddata.no) {
    console.log(
      `이름 : ${data.ename} 직급 : ${data.title} 부서번호 : ${data.dno} 부서명 : ${ddata.dname}`
    );
  }
});
