...
// Object 타입 -> java Map : key - value 이뤄져있다.

// const dov = { key: 단일값만 원시값(: 주의!! String은 JS에서 원시값임)만 들어갈 수 있는데 왠만하면 String 만 사용하자
//   name: '바둑이',
//   age: 4,
//   isOwner: true
// }
...
dog = {
  ...
  age: 4,
  isOwner: true
}; // 그냥 쓰면 String 으로 자동으로 key 값 인식한다.
// console.log(dov); // {name: '바둑이', age: 4, isOwner: true}

// 오브젝트 값(속성값) 접근 방법 2가지

console.log(dog.name);
console.log(dog["name"]); // dog[name] 하면 에러 뜬다 꼭 String으로 key 값 넣어주기
...
function getKeyValue2(object, key) {
  console.log(object.key); // 순수하게 key 하는 속성값을 객체 안에서 찾는것 : 동적으로 못바꾼다.
...
getKeyValue2(dog, 'isOwner');
getKeyValue2(dog, 'name');
